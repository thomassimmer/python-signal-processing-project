...
signal_demodulated = filters.butter_lowpass_filter(multiplied_signal, 1000, samplingRate)
signal_demodulated = filters.butter_highpass_filter(signal_demodulated, 10, samplingRate)
...
r = threshold_radius
for symbol in symbols.symbols_32APSK:
    plt.plot(symbol.number.real + r*np.cos(theta), symbol.number.imag + r*np.sin(theta), 'black')
